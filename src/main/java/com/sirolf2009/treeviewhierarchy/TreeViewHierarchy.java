 ...
 * </p>
 * <p>
 * Each item in the list must implement the {@link IHierarchicalData} interface, in order to map the recursive nature of the tree data to the tree view.
 * </p>
 * <p>
 * Each change in the underlying data (adding, removing, sorting) will then be automatically reflected in the UI.
 * </p>
 *
 * @author Christian Schudt http://myjavafx.blogspot.nl/2012/03/treeview-with-data-source.html
 */
public class TreeViewHierarchy<T extends IHierarchicalData<T>> extends TreeView<T> {
	 ...
	 */
	public TreeViewHierarchy(TreeItem<T> root) {
		super(root);
		init();
	}
	...
	private void clear(TreeItem<T> root) {
		if (root != null) {
			for (final TreeItem<T> treeItem : root.getChildren()) {
	...
	private void updateItems() {
		...
		if (getItems() != null) {
			for (final T value : getItems()) {
				getRoot().getChildren().add(addRecursively(value));
			}

			final ListChangeListener<T> rootListener = getListChangeListener(getRoot().getChildren());
			final WeakListChangeListener<T> weakListChangeListener = new WeakListChangeListener<>(rootListener);
			...
			public void onChanged(final Change<? extends T> change) {
				while (change.next()) {
					...
					if (change.wasPermutated()) {
						// Store the new order.
						final Map<Integer, TreeItem<T>> tempMap = new HashMap<>();

						for (int i = change.getTo() - 1; i >= change.getFrom(); i--) {
							final int a = change.getPermutation(i);
	...
	private TreeItem<T> removeRecursively(TreeItem<T> item) {
		if (item.getValue() != null && item.getValue().getChildren() != null) {
			...
			for (final TreeItem<T> treeItem : item.getChildren()) {
				removeRecursively(treeItem);
			}
		}
		return item;
	...
	private TreeItem<T> addRecursively(T value) {
		final TreeItem<T> treeItem = new TreeItem<>();
		treeItem.setValue(value);

		if (value != null && value.getChildren() != null) {
			final ListChangeListener<T> listChangeListener = getListChangeListener(treeItem.getChildren());
			final WeakListChangeListener<T> weakListener = new WeakListChangeListener<>(listChangeListener);
			...
			for (final T child : value.getChildren()) {
